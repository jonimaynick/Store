<%-- 
    Document   : admin
    Created on : Jun 21, 2022, 2:04:32 AM
    Author     : PC
--%>

<%@page import="phu.DAO.BookDAO"%>
<%@page import="phu.DTO.BookDTO"%>
<%@page import="java.util.List"%>
<%@page import="phu.DTO.UserDTO"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <link href="header.css" rel="stylesheet" type="text/css"/>
        <title>Admin Page</title>
    </head>
    <body>
                <div class="header">
            <div class="mainMenu">
                <ul class="menu">
                    <a href="home.jsp" >
                        <img id="logo" src="data:image/png;base64,"
                             width="80" height="80" />                                
                    </a>
                    <li>
                        <form action="MainController" method="POST">
                            <input type="submit" name="action" value="Home"/>
                        </form>
                    </li>
                    <li>
                        <form action="MainController" method="POST">
                            <a href="aboutUs.jsp"  style="text-decoration: none; color: white; font-weight: bold; font-size: 23px;">About Us</a>
                        </form>
                    </li>

                    <%
                        UserDTO user = (UserDTO) session.getAttribute("LOGIN_USER");
                        if (user == null || user.getRoleName().equals("USER")) {
                    %>
                    <li>

                        <form action="MainController" method="POST">
                            <a href="cart.jsp"  style="text-decoration: none; color: white; font-weight: bold; font-size: 23px;">Cart</a>
                        </form>
                    </li>
                    <%
                        }
                    %>
                </ul>
                <div class="user">
                    <ul class="menu-user">

                        <%
                            if (user == null) {
                        %>
                        <li>
                            <a>Guest</a>
                        </li>
                        <li>
                            <form action="MainController" method="POST">
                                <input type="submit" name="action" value="Login"/>
                            </form>
                        </li>
                        <%
                        } else {
                        %>
                        <li>
                            <a><%=user.getUserName()%></a>
                        </li>
                        <li>
                            <form action="MainController" method="POST">
                                <input type="submit" name="action" value="Logout"/>
                            </form>
                        </li>
                        <%
                            }
                        %>
                    </ul>
                </div>
            </div>
        </div>
        <div class="headerLine"></div>
        <div class="main" style="text-align: center; margin: 10px;">
            <a href="addBook.jsp" style="text-decoration: none; border: 1px solid; background: #cccccc;  padding: 5px;">add new book</a>
            <form action="MainController" method="GET" style="margin: 10px;">
                <input type="text" value="" name="txtSearch">   
                <input type="submit" value="Search" name="Action">
            </form>

            <div class="table">
                <table border="1">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Book ID</th>
                            <th>Title</th>
                            <th>Price</th>
                            <th>Image</th>
                            <th>Describe</th>
                            <th>Author</th>
                            <th>Category</th>
                            <th>Quantity</th>
                            <th>Import Date</th>
                            <th>Status</th>
                            <th>Update</th>
                            <th>Delete</th>
                        </tr>
                    </thead>
                    <tbody>
                        <%
                            List<BookDTO> bookList = (List<BookDTO>) session.getAttribute("ListSearch");
                            BookDAO daoBook = new BookDAO();
                            if (bookList == null) {
                                bookList = daoBook.searchAllBooks("");
                            }
                            int count = 0;
                            for (BookDTO dto : bookList) {
                                count++;
                        %>
                    <form action="MainController" method="GET">
                        <tr>

                            <td><%=count%></td>
                            <td>
                                <input type="text" value="<%=dto.getBookID()%>" readonly="true" name="BookID" required="true">
                            </td>
                            <td>
                                <input type="text" value="<%=dto.getTitle()%>" name="BookTitle" required="true">
                            </td>
                            <td>
                                <input type="number" value="<%=dto.getPrice()%>" name="BookPrice" min="0" required="true">
                            </td>
                            <td>
                                <img src="<%=dto.getImage()%>"  height="60" width="50"/>
                                <input type="text" value="<%=dto.getImage()%>" name="BookImage" required="true">
                            </td>
                            <td>
                                <input type="text" value="<%=dto.getDescribe()%>" name="BookDescribe" required="true">
                            </td>
                            <td>
                                <input type="text" value="<%=dto.getAuthor()%>" name="BookAuthor" required="true">
                            </td>
                            <td>
                                <input type="text" value="<%=dto.getCategory()%>" name="BookCategory" readonly="" >
                            </td>
                            <td>
                                <input type="number" value="<%=dto.getQuantity()%>" name="BookQuantity" min="0" required="true">
                            </td>
                            <td>
                                <input type="text" value="<%=dto.getImportDate()%>" name="importDate" readonly="true" required="true">
                            </td>
                            <td>
                                <%=dto.isStatus()%>
                            </td>
                            <td>
                                <input type="submit" name="action" value="Update"/>
                            </td>
                            <td>
                                <script>
                                    function ConfirmDelete(e) {
                                        var result = confirm("Do you want to delete book ?");
                                        if (result) {

                                        } else {
                                            e.preventDefault();
                                        }
                                    }
                                </script>
                                <%
                                    if (dto.isStatus()) {
                                %>
                                <input id="deleteBook" type="submit" name="action" value="Delete" onclick="ConfirmDelete(event)"/>
                                <%
                                    }
                                %>

                            </td>

                        </tr>
                    </form>
                    <%
                        }
                    %>

                    </tbody>
                </table>
            </div>
        </div>
    </body>
</html>
