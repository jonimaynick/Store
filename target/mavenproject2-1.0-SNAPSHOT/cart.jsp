<%-- 
    Document   : cart
    Created on : Jun 21, 2022, 2:05:19 AM
    Author     : PC
--%>

<%@page import="java.util.ArrayList"%>
<%@page import="phu.DTO.BookDTO"%>
<%@page import="phu.DTO.BookError"%>
<%@page import="java.util.List"%>
<%@page import="phu.DTO.CartDTO"%>
<%@page import="phu.DTO.UserDTO"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>My Cart</title>
        <link href="header.css" rel="stylesheet" type="text/css"/>
        <link href="cart1.css" rel="stylesheet" type="text/css"/>
    </head>
    <body>
        <div class="container">
            <div class="header">
                <div class="mainMenu">
                    <ul class="menu">
                        <a href="home.jsp" >
                            <img id="logo" src="data:image/png;base64,"
                                 width="80" height="80" />                                
                        </a>
                        <li>
                            <form action="MainController" method="POST">
                                <input type="submit" name="action" value="Home"/>
                            </form>
                        </li>
                        <li>
                            <form action="MainController" method="POST">
                                <a href="aboutUs.jsp"  style="text-decoration: none; color: white; font-weight: bold; font-size: 23px;">About Us</a>
                            </form>
                        </li>
                        <li>

                            <form action="MainController" method="POST">
                                <!--                                <input type="submit" name="action" value="Cart"/>-->
                                <a href="cart.jsp"  style="text-decoration: none; color: white; font-weight: bold; font-size: 23px;">Cart</a>
                            </form>
                        </li>
                    </ul>
                    <div class="user">
                        <ul class="menu-user">

                            <%
                                UserDTO user = (UserDTO) session.getAttribute("LOGIN_USER");
                                if (user == null) {
                            %>
                            <li>
                                <a>Guest</a>
                            </li>
                            <li>
                                <form action="MainController" method="POST">
                                    <input type="submit" name="action" value="Login"/>
                                </form>
                            </li>
                            <%
                            } else {
                            %>
                            <li>
                                <a><%=user.getUserName()%></a>
                            </li>
                            <li>
                                <form action="MainController" method="POST">
                                    <input type="submit" name="action" value="Logout"/>
                                </form>
                            </li>
                            <%
                                }
                            %>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="headerLine"></div>
            <div class="mainPage">

                <%
                    CartDTO cart = (CartDTO) session.getAttribute("CART");
                    if (cart == null || cart.isEmpty()) {
                %>
                <h2>No cart</h2>
                <%
                } else {
                    float total = 0;
                    List<BookError> errorList = (List<BookError>) request.getAttribute("QUANTITY_ERROR");
                    if (errorList == null) {
                        errorList = new ArrayList<BookError>();
                    }
                    for (BookDTO dto : cart.getCart().values()) {
                        total += dto.getPrice() * dto.getQuantity();
                %>
                <div class="product-line">
                    <img src="<%=dto.getImage()%>" width="80px" height="100px"/>
                    <div id="book-information">
                        <h4><%=dto.getTitle()%></h4>
                        <form action="MainController">
                            <input type="hidden" name="bookID" value="<%=dto.getBookID()%>"/>
                            <input type="number" name="quantity" value="<%=dto.getQuantity()%>"/>
                            <%
                                for (BookError err : errorList) {
                                    if (err.getBookID().equals(dto.getBookID())) {
                            %>
                            <h4 style="color: red;"><%=err.getError()%></h4>
                            <%
                                    }
                                }
                            %>

                            <input type="number" name="price" value="<%=dto.getPrice()%>" readonly="true"/>
                            <input type="number" name="totalPrice" value="<%=dto.getPrice() * dto.getQuantity()%>" style="color: red;" readonly="true"/>
                            <input type="submit" name="action" value="Update Cart"/>
                            <input type="submit" name="action" value="Delete Cart"/>
                        </form>
                    </div>
                </div>
                <%
                    }
                %>
                <h3 id="total" >Total Price : <%=total%></h3>
                <%
                    String couponsCode = (String) request.getAttribute("COUPONSCODE");
                    if (couponsCode == null) {
                        couponsCode = "";
                    }
                    String stringPercent = (String) request.getAttribute("PERCENT_DISCOUNT");
                    int percent = 101;
                    if (stringPercent != null) {
                        percent = Integer.parseInt(stringPercent);
                    }

                    if (percent <= 100 && percent >= 0) {
                %>

                <style>
                    #total{
                        text-decoration: line-through;  
                    }
                </style>
                <h2 style="color:red">=> <%=total - (total * percent / 100)%></h2>

                <%
                    }
                %>

                <form action="MainController" method="POST">
                    <input type="text" name="Coupons" value="<%=couponsCode%>"/>
                    <input type="submit" name="action" value="Check Coupons"/><br><br>


                    <%
                        if (user == null || user.getRoleName().equals("ADMIN")) {
                    %>


                    <h4 style="color: red">Please login by user to payment!</h4>
                    <%
                    } else {
                    %>
                    <input type="submit" name="action" value="Payment"/>
                    <%
                            }
                        }
                    %>
                </form> 

            </div>
        </div>
    </body>
    <style>
        .mainPage{
            margin: 10px auto;
        }
        .product-line{
            margin: 5px 5px;
            border: 1px solid;
        }
        #book-information{
            margin: 0px 10px;
        }
        .product-line{
            display: flex;
        }
    </style>
</html>
